...
-- Mine
local ICONS = {
	[136998] = "Interface\\AddOns\\ls_Toasts_Classic\\assets\\PVPCurrency-Honor-Alliance",
	[137000] = "Interface\\AddOns\\ls_Toasts_Classic\\assets\\PVPCurrency-Honor-Horde",
}
...
local function Toast_SetUp(event, id, quantity)
	...
	if isNew then
		...
		toast.Icon:SetTexture(ICONS[info.icon] or info.icon)
		...
		toast._data.id = id
		toast._data.sound_file = C.db.profile.types.loot_currency.sfx and "Interface\\AddOns\\ls_Toasts_Classic\\assets\\ui-common-loot-toast.OGG"
...
local function updateFilterOptions()
	...
	for id in next, C.db.profile.types.loot_currency.filters do
		...
		options[tostring(id)] = {
			...
			type = "group",
			name = ("|T%s:0:0:0:0:64:64:4:60:4:60|t %s"):format(ICONS[info.icon] or info.icon, info.name),
			args = {
				desc = {
					order = 1,
					type = "description",
...
local function Enable()
	if C.db.profile.types.loot_currency.enabled then
		ICONS[4006481] = "Interface\\AddOns\\ls_Toasts_Classic\\assets\\PVPCurrency-Conquest-" .. UnitFactionGroup("player")
...
local function Test()
	local honorID = Constants.CurrencyConsts.CLASSIC_HONOR_CURRENCY_ID
	if not C.db.profile.types.loot_currency.filters[honorID] then
		C.db.profile.types.loot_currency.filters[honorID] = 0

		updateFilterOptions()
	end

	local arenaID = Constants.CurrencyConsts.CLASSIC_ARENA_POINTS_CURRENCY_ID
	if not C.db.profile.types.loot_currency.filters[arenaID] then
		C.db.profile.types.loot_currency.filters[arenaID] = 0

		updateFilterOptions()
	end

	Toast_SetUp("LOOT_CURRENCY_TEST", honorID, m_random(-250, 250))
	Toast_SetUp("LOOT_CURRENCY_TEST", arenaID, m_random(-250, 250))
end
