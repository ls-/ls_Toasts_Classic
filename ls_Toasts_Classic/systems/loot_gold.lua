...
E:RegisterOptions("loot_gold", {
	...
	disabled = function(info)
		if info[#info] == "loot_gold" then
			return false
		else
			return not C.db.profile.types.loot_gold.enabled
		end
	end,
	get = function(info)
	...
	args = {
		...
		threshold = {
			...
			disabled = function()
				return not C.db.profile.types.loot_gold.enabled or C.db.profile.types.loot_gold.track_loss
			end,
			get = function()
